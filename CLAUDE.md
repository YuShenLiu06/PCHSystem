...
> 已完成的工作（按版本 / release 组织）见 [`CHANGELOG.md`](./CHANGELOG.md)——`[Unreleased]` 含最近迭代（submit-batch 批量端点、施工进度上报层迭代 1~3、积分层首批 credit/debit/ledger），`vX.Y.Z` 段含历史发版；具体雷点见各子服务 `CLAUDE.md` 与 [`Docs/architecture/`](./Docs/architecture/)。本节只列**未完成**项。
...
- [ ] **积分层后续**：`/me/scores` 汇总端点（JWT）、settle 编排（Calculator 链 + 归档 hook 接线）、玩家消耗积分端点（禁透支）、公开榜单。首批 credit/debit/ledger REST 端点 + 迁移 0024 + **积分管理面板**（前端 `/admin/scoring` 双 tab「积分流水/玩家积分」；后端 `admin/adjust` 仅特权 JWT + `admin/players` 联想 + `admin/balances` 余额排名 + `ADMIN_*` env 托管账号，见 [`api/scoring.md`](./Docs/architecture/api/scoring.md)）已落地。
