...
*最后更新：2026-07-19（身份主锚升级：Web 账号绑定多 MC UUID / 完善 `!!PCH bind`——后端迁移 0014 `web_accounts`+`bind_tokens` + 8 端点 + JWT sub 改 account_id + role 迁 account 级 + 聚合按 account（业务表零迁移）；MCDR `!!PCH bind` 双向（无参出码 / `<code>` 消费）；前端身份管理 5 页 + `/me` 升级。R-5 落地）*
