...
| `!!PCH sheet list [--mine]` / `view` / `create` / `add` / `set` / `delrow` / `claim` / `deliver` / `done` / `progress` / `contribute` / `release` / `reject` / `notify list` | user / owner | 在线表格协作（service-token + `X-Player-UUID` 代玩家写；`deliver` 按 mode 分流——lock→`/delivery` 绝对值 / progress→`/contribute` 增量；`progress` = owner 调整 progress 进度绝对值；命令↔HTTP↔角色映射见 [`api/sheets.md`](../Docs/architecture/api/sheets.md) §11） |
...
*最后更新：2026-07-03（v0.3.0：deliver 按 mode 分流到 contribute/delivery + 贡献者显示 + 特权按钮按查看者身份显隐）*

*增量（2026-07-03 bugfix）：新增 `!!PCH sheet progress` 命令（owner 调整 progress 进度绝对值，view 中 progress 行 owner 可见 `[调整进度]` 按钮）；后端 owner 写操作（release / progress / upsert 改 mode）现通知 progress 行贡献者（新 category `sheet_progress_changed` / `sheet_progress_reset`）。*

*增量（2026-07-05）：sheet view tellraw 美化——新增 `text_layout.py`（像素宽度估算 + 右对齐/居中填充，`text_width_px`/`right_align_suffix`/`center_leading`，§码 0 宽 + §l 粗体 +1px/char + 颜色码重置粗体）+ `format_section_separator` 分节标题（gold+bold §6§l，走 §6 色板「重要/标题」槽）+ `format_submit_footer`（公开 `[一键提交]`）+ owner 行按钮 `[改ID]`；测试 stub 补 `to_plain_text()`，MCDR 测试 113 绿。*
