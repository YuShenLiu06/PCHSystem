...
*最后更新：2026-07-09（子物品嵌套行 issue #19 + sheets.py 包化重构：新增 `addsub`/`delsub`/`setsub` 命令；`messages.py` 缩进渲染子行 + 父行 `+N子件` 徽标；按钮紧凑化（单字 `[认][改][-][+]` + RText hover）；`scanner.py` 不改（子行同列表自动匹配）；后端包化拆分 `sheets/` 包 + `translation.py` 公共翻译；详见 [`api/sheets.md`](../Docs/architecture/api/sheets.md) §14 增量日志）*
