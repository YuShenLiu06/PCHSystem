...
| `!!submit` / `!!submit <sheet_id>` | user | **一键提交新根**（feat a619510）：`!!submit` 为**第三命令根**（独立 help message，镜像 `!!sheet` 多根注册），无参 = 重开上次查看的表格并直接提交（复用 `!!sheet` 的 `GET /me/last_sheet` 存储，后端零改动）；`!!submit <编号>` 指定表格。与 `!!PCH sheet submit <id>` 共用 `_sheet_submit_impl` 实现；回执仅逐行展示与本人相关的跳过行（本人认领的 lock 行、progress 已备齐/无需求），其余（他人认领的 lock 行、progress 未携带项）折叠为末尾一行计数（bugfix） |
...
*最后更新：2026-07-11（`!!submit` 一键提交新根 + 回执折叠降噪：新增 `!!submit` 第三命令根（无参重开上次表格直接提交，有参指定表），复用 `!!sheet` 的 `GET /me/last_sheet` 存储；回执仅逐行展示与本人相关的跳过行（本人认领的 lock 行、progress 已备齐/无需求），其余（他人认领的 lock 行、progress 未携带项）折叠为末尾一行计数 `SHEET_SUBMIT_FOLDED_LINE`）*
