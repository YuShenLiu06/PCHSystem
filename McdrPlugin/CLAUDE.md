...
| `!!submit` / `!!submit <sheet_id>` | user | **一键提交**（feat）：`!!submit` 重开上次表格并直接提交（复用 `!!sheet` 的 `last_sheet` 存储）；`!!submit <编号>` 指定表格。与 `!!PCH sheet submit <id>` 共用实现；回执仅逐行展示与本人相关的跳过行，其余折叠为一行计数（bugfix） |
