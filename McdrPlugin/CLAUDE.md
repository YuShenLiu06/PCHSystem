...
- 入口：`McdrPlugin/htcmc_auth/htcmc_auth/__init__.py`（`on_load` 注册命令树）
- 命令回调：`htcmc_auth/commands.py`（`_pch_root` / `_login` / `_not_impl`）
- 消息/色彩常量：`htcmc_auth/messages.py`
- 元数据：`McdrPlugin/htcmc_auth/mcdreforged.plugin.json`
- HTTP 客户端：`htcmc_auth/client.py`（`LoginResult` dataclass）
- 配置：`htcmc_auth/config.py` + `config.json`
...
*最后更新：2026-07-02（由 service-claude-md skill 逻辑手动生成；下次更新请优先调用 service-claude-md skill）*
