...
| `!!PCH sheet list [--mine]` / `view` / `create` / `add` / `set` / `delrow` / `claim` / `deliver` / `done` / `contribute` / `release` / `reject` / `notify list` | user / owner | 在线表格协作（service-token + `X-Player-UUID` 代玩家写；`deliver` 按 mode 分流——lock→`/delivery` 绝对值 / progress→`/contribute` 增量；命令↔HTTP↔角色映射见 [`api/sheets.md`](../Docs/architecture/api/sheets.md) §11） |
