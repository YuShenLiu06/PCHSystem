...
*最后更新：2026-07-06（sheet 快速重开 + list 简写旗标：新增 `!!sheet` 第二命令根（无参重开上次 / `<id>` 直开）+ `!!PCH sheet last` 别名，经 `GET /me/last_sheet` 取 `last_sheet_id`；`sheet list` 默认 active + 参与优先 + 阶段标签 `format_phase_label` + 单字母简写旗标 `-m/-c/-t/-a/-l` 可组合；纯函数 `_parse_list_flag_tokens` 单测）*
