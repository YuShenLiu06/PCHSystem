...
| 自动阈值判模式（已否决，改拥有者手控） | 进度模式的「众筹」（谁都能交付，已否决，改单认领人跟踪 delivered_qty） |
...
| D-4 | 进度模式 = **单认领人跟踪 delivered_qty**（非众筹） | 与 lock 一致的认领基座；未来多人 = 多人各自跟踪 |
...
- **进度模式「众筹」**：已否决（D-4）；若未来需要任意玩家上报，再扩展。
