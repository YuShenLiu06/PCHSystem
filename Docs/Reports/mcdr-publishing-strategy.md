...
1. **`on_load` 启动自检**（最关键）：config 缺失 / `api_url`+`service_token` 为默认值 / `GET <api_url>/health` 不通 → MCDR 控制台醒目 WARNING + 仓库链接 + `!!PCH setup` 指引。插件**不崩**（`!!MCDR plugin list` 照常显示已加载），但"你需要配我"不可能看不见。
...
### 6.2 启动自检（去喷化核心）
- [ ] 在 `on_load` 加 config 存在性 + 默认值 + `/health` 连通性三检；不通则醒目 WARNING + 指引。复用 `client.py` / `sheet_client.py` 的 HTTP 管道，注意放 `@new_thread` 或非阻塞（R-12）。
