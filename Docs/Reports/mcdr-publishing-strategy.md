...
`htcmc_auth` 是 **PCHSystem 后端的"游戏端客户端"**（R-7 瘦客户端），所有命令（login/bind/sheet/project/score/title/notify）都是对后端的 HTTP 调用。希望将来把它发到 [MCDR 官方 catalogue](https://github.com/MCDReforged/PluginCatalogue) 获得曝光、吸引贡献者；**后端个人自用，不对外提供服务。**
...
| **MCDR tag 约定** | 改为 `htcmc_auth-vX.Y.Z`（catalogue 合法格式 4，前向兼容）。 |
...
- **Release 自动探测条件**（catalogue 从你仓库的 GitHub Release 抓 `.mcdr` 下载链接）：
  ...
  2. **tag 名匹配版本**，仅四种合法格式：
     - `<version>`（`1.2.3`）
     - `v<version>`（`v1.2.3`）
     - `<plugin_id>-<version>`（`htcmc_auth-1.2.3`）
     - `<plugin_id>-v<version>`（`htcmc_auth-v1.2.3`）
  3. 附件含 `.mcdr` 或 `.pyz`。
...
- `McdrPlugin/htcmc_auth/mcdreforged.plugin.json`：`link` **指向错误仓库**（`gubaiovo/MCDR_uuid_api_remake`）；**缺 `archive_name` / `resources`**。
- `McdrPlugin/htcmc_auth/tests/test_scanner.py`：**散落在打包根**，会被打入 `.mcdr`，需移走。
- 插件依赖另两个 MCDR 插件 `minecraft_data_api`、`uuid_api_remake`（非 pip 包）。
- 仓库**完全没有 `.github/`**，无任何 CI。
- 现有 tag 约定 `mcdr-vX.Y.Z` **不命中 catalogue 任何一种合法格式**（解析出的 plugin_id 是 `mcdr` ≠ `htcmc_auth`），Release 不会被探测。
...
1. **`on_load` 启动自检**（最关键，**✅ 已实现 2026-07-12**）：落地于 `McdrPlugin/htcmc_auth/htcmc_auth/health.py` + 后端 `GET /info`（返 `version` + `web_base_url`）。状态分档：
   ...
   - **插件自身**：始终置顶一行 `htcmc_auth v<version>`（`PluginServerInterface.get_plugin_metadata` 取 `mcdreforged.plugin.json`，best-effort 回落「版本未知」）；报告末尾固定作者页脚 `作者：<author>`（元数据 `.authors`，回落 `YuShen`）。
...
### 6.1 插件元数据修正
- [ ] `McdrPlugin/htcmc_auth/mcdreforged.plugin.json`：`link` 改为本仓库正确 URL；加 `archive_name`（如 `htcmc_auth-v{version}`）；按需加 `resources`。
- [ ] 把散落的 `McdrPlugin/htcmc_auth/tests/test_scanner.py` 移到 `McdrPlugin/tests/`，避免被打入包。
...
- [x] `on_load` 自检：`McdrPlugin/htcmc_auth/htcmc_auth/health.py`（`probe_backend` 探 `/info`、404 回退 `/healthz`；`probe_frontend` 按 `web_base_url` 探；`classify` 状态分档；`run_console_check` `@new_thread` best-effort 吞异常）+ `__init__.py:_start_health_check`。
...
- [x] 插件自身版本 + 作者页脚：`resolve_plugin_meta`（`get_plugin_metadata("htcmc_auth")`，S-1 已核实）→ 报告置顶 `htcmc_auth v<ver>` finding + 末尾 `作者：<author>`。
- [ ] `!!PCH setup` 交互式向导（收 `api_url`+`token` 写 config）—— 仍待办（本次只做「检测+提醒」）。

### 6.3 `!!PCH setup` 向导
- [ ] 新增命令子树（`Literal("setup")`），交互收 api_url + token，调 `/health` 自检，写 `config/htcmc_auth/config.json`，成功后回执。
...
- [ ] `.github/workflows/release.yml`：on `htcmc_auth-v*` tag → `pip install mcdreforged` → `mcdreforged pack -i McdrPlugin/htcmc_auth` → `softprops/action-gh-release@v2` 挂 `./*.mcdr` + 从 `CHANGELOG.md` 抽 notes。
- [ ] backend / frontend 的 tag（`backend-v*` / `frontend-v*`）：只生成 release notes，不发镜像、不打 artifact（部署从源码 compose build）。

### 6.6 tag 约定
- [ ] `CONTRIBUTING.md` §4：`mcdr-vX.Y.Z` → `htcmc_auth-vX.Y.Z`（catalogue 合法、前向兼容）。

### 6.7 catalogue 提交（外部 PR，手动一次性）
- [ ] Fork `MCDReforged/PluginCatalogue`，在 `plugins/htcmc_auth/` 建 `plugin_info.json`（id/authors/repository/branch/labels=intro+tool/introduction）+ `introduction.md`（前置诚实）。
- [ ] 提 PR，附本仓库链接，等维护者审核。
