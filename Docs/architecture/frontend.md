...
| 提交·积分·榜单 | 积分管理 `ScoringAdmin.vue`（`/admin/scoring`，admin/owner：el-tabs 双 tab「积分流水」（玩家/时间筛选 + 服务端分页）/「玩家积分」（`ScoringBalances.vue` 余额排名，调分后即时刷新）+ 单条调分弹窗）；提交审计、公开榜单（规划） | scoring-service |
