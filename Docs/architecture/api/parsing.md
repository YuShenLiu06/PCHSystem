...
| POST | `/sheets/from-items` | `get_current_player` | `SheetFromItemsRequest{title, items[]}` | 201 `SheetDetail` | 一次性建表 + 批量行（mode 默认 lock）。见 [`sheets.md`](./sheets.md) |
...
PreviewItem(item_id, item_name, count) + untranslated[]
   │  前端预览 → 确认
   ▼
POST /sheets/from-items   →   sheets 协作流
```
