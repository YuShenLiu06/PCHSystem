...
**状态**：✅ 已实现（后端 + 前端 + MCDR 默认追踪器 v0.9.0）。剩余 TODO（switch-self mod_id 归属校验、mod-token 签发流）见 §6 / CLAUDE.md §7。
...
- [3. 上报（`POST /v1/construction/report`）](#3-%E4%B8%8A%E6%8A%A5post-v1constructionreport)
  - [3.1 严格单源（C-7）](#31-%E4%B8%A5%E6%A0%BC%E5%8D%95%E6%BA%90c-7)
  - [3.2 方块清单校验](#32-%E6%96%B9%E5%9D%97%E6%B8%85%E5%8D%95%E6%A0%A1%E9%AA%8C)
  - [3.3 按材料封顶](#33-%E6%8C%89%E6%9D%90%E6%96%99%E5%B0%81%E9%A1%B6)
- [4. 归因查询与进度展示（GET 端点）](#4-%E5%BD%92%E5%9B%A0%E6%9F%A5%E8%AF%A2%E4%B8%8E%E8%BF%9B%E5%BA%A6%E5%B1%95%E7%A4%BAget-%E7%AB%AF%E7%82%B9)
  - [4.1 时序快照表（placement\_snapshots）](#41-%E6%97%B6%E5%BA%8F%E5%BF%AB%E7%85%A7%E8%A1%A8placementsnapshots)
  - [4.2 加入施工端点行为](#42-%E5%8A%A0%E5%85%A5%E6%96%BD%E5%B7%A5%E7%AB%AF%E7%82%B9%E8%A1%8C%E4%B8%BA)
  - [4.3 个人查询端点行为](#43-%E4%B8%AA%E4%BA%BA%E6%9F%A5%E8%AF%A2%E7%AB%AF%E7%82%B9%E8%A1%8C%E4%B8%BA)
- [5. 默认追踪器实现契约](#5-%E9%BB%98%E8%AE%A4%E8%BF%BD%E8%B8%AA%E5%99%A8%E5%AE%9E%E7%8E%B0%E5%A5%91%E7%BA%A6)
- [6. 切源端点（D9）](#6-%E5%88%87%E6%BA%90%E7%AB%AF%E7%82%B9d9)
- [7. 归档 / 结算接入契约](#7-%E5%BD%92%E6%A1%A3--%E7%BB%93%E7%AE%97%E6%8E%A5%E5%85%A5%E5%A5%91%E7%BA%A6)
...
- [11. 数据模型（迁移 0018-0023）](#11-%E6%95%B0%E6%8D%AE%E6%A8%A1%E5%9E%8B%E8%BF%81%E7%A7%BB-0018-0023)
...
| PATCH | `/mod-sources/{name}` | admin | 逐源启停 `{enabled}` |
...
| GET | `/me/reports` | 任意 auth player（须绑账号） | 个人上报历史（`placement_snapshots` 投影） |
| **GET** | **`/me/report-events`** | 任意 auth player（须绑账号） | **个人完整事件流水（`accepted` + 所有 `skipped` reason）** |
| **GET** | **`/me/construction`** | 任意 auth player | **查自己当前活跃加入的施工项目** |
| **POST** | **`/me/join`** | 任意 auth player（须绑账号） | **手动加入 sheet** |
| **POST** | **`/me/switch`** | 任意 auth player（须绑账号） | **切换到指定 sheet（leave + join 同事务）** |
| **POST** | **`/me/leave`** | 任意 auth player | **退出当前活跃加入（幂等空态）** |
| **POST** | **`/active-by-uuids`** | service-token 单头 | **批量 UUID → 活跃 sheet_id（tracker 按玩家路由用，非敏感）** |

admin 端点挂 `require_role("admin")`——**account 级 JWT**（仅 `Authorization: Bearer`，admin ≠ service-token，与 scoring `require_privileged_access` 同范式；role 权威源 `WebAccount.role`，**无绑定玩家的托管账号（`ADMIN_*` env，JWT 无 `active_uuid`）可正常调用**，issue #74；非 admin/owner → 403，R-9/RS-2）。`GET /{sheet_id}/progress` 走 `get_current_viewer`（双通道，player-less 托管账号亦可浏览）。
...
- skip reason 全集：`当前无施工中项目` / `多个施工项目并发，须显式指定 sheet_id` /
  ...
  `客户端模组上报已被服主关闭` / `方块不在项目材料清单内`（见 §3.2）/
  `已达材料上限`（见 §3.3）。

每条 placement 按以下 4 门**顺序**校验，任一门未过 → 该条 skip（reason 取决于卡在哪道门）：

```mermaid
flowchart TD
    R["report 到达"] --> G1{"① 归因\nsheet_id 有效?"}
    G1 -- "否" --> S1["skip：无施工中项目 / 须显式指定 / 非 constructing"]
    G1 -- "是" --> G2{"② 单源 C-7\n= 玩家活跃源?"}
    G2 -- "否" --> S2["skip：由其他源上报 / 无活跃源"]
    G2 -- "是" --> G3{"③ 清单 §3.2\nregistry_id ∈ 清单?"}
    G3 -- "否" --> S3["skip：方块不在材料清单内"]
    G3 -- "是" --> G4{"④ 封顶 §3.3\n跨账号合计 < need?"}
    G4 -- "已满" --> S4["skip / 部分接受：已达材料上限"]
    G4 -- "是" --> OK["✅ accepted — 落库"]
```
...
每条 placement 落库前，校验 `registry_id` 必须在该 sheet 的收集清单内——
即 `sheet_rows.registry_id` 集合（**含子物品**：顶层行的 `registry_id` + 子行的 `registry_id`）。
不在清单内的方块 → `action="skipped"`（reason `方块不在项目材料清单内`），**不落库**。

设计动机：与 C-6 叠加双保险——追踪器侧自过滤（C-6）+ 后端按清单再挡。追踪器漏过或恶意客户端 mod 上报非清单方块时，后端拦下，不污染 `placement_records`。

> 清单不可省：若该 sheet 的 `sheet_rows.registry_id` 集合为空（空项目/异常），所有 placement 全 skip。
...
**跨账号合计净量不得超过该材料需求总量**——`sum(placement_records.net_qty) WHERE sheet_id=$sheet AND registry_id=$rid` 上限 = `sum(sheet_rows.need_qty)`（含子物品 `registry_id`，与 §4 `material_completion` 同口径）。

逐条处理（[`construction_repo.submit_report`](../../../Backend/app/repositories/construction_repo.py) §「按材料封顶」分支）：

```mermaid
flowchart TD
    P["一条 placement\ndelta = placed − broken"] --> Q1{"delta > 0?"}
    Q1 -- "否（拆毁/中性）" --> A0["接受 + 释放容量\nnet_delta = delta"]
    Q1 -- "是（净放置）" --> Calc["available = max(need − 当前跨账号合计, 0)"]
    Calc --> Q2{"available"}
    Q2 -- "= 0" --> S0["整条 skip\nreason: 已达材料上限"]
    Q2 -- "> 0" --> Q3{"delta ≤ available?"}
    Q3 -- "是" --> F0["✅ 全额 accepted\nnet_delta = delta"]
    Q3 -- "否" --> P0["部分 accepted + emit skipped(over)\n/me/report-events 可见"]
```

> 同批次内 `material_totals` 是逐条累加的内存字典——本批先后顺序影响单条可用量；下一批次重新从 DB 读最新合计。拆毁后再次放置可重新吃回释放的容量。

设计动机：避免超量数据污染积分——归档时 `BuildAScoreCalculator` 读 `placement_records` 聚合，若某材料合计 > 需求会过度奖励。封顶由后端兜底，无需追踪器侧实现。
...
- `material_completion`：材料完成度——按 `registry_id` 聚合 placement 并对照该 sheet 的 `sheet_rows` 清单（含子物品 `registry_id`），结构：
  ```json
  [
...
- `timeline`：时序快照——按 `construction.placement_snapshots` 表升序 limit 200，结构 `[{account_id, total_net, recorded_at}]`，每次 report 落库后为 accepted 的 account 各写一条（详见 §4.1）。
  - 展示用、非权威：失败仅日志、不阻断 report；缺数据时前端按 `account_totals` 兜底（折线退化为单点）。
- `construction_started_at` / `archived_at`（迁移 0020 加 `sheets.sheets.constructing_at`）：分别取施工开始时间 + 归档时间。供前端折线图 xAxis 范围（左沿贴施工开始；右沿停在归档或当前时间）+ 归档 timeline 点亮「进入施工」段。直跳归档（`collecting → archived` 跳过 constructing）时 `construction_started_at = null`。回填策略见 §11.1。

### 4.1 时序快照表（placement_snapshots）
...
**与归档/结算的关系**：归档时 `placement_snapshots` 随 sheet CASCADE 删除。timeline 不参与积分计算（归档仅读 `placement_records` 聚合）。
...
两端点均查 `limit`（默认 50，`1..200`），按时间倒序，跨所有项目。归因锚 account（R-5）：未绑 Web 账号 → 403。展示用（权威源是 `placement_records`）。
...
**`/me/report-events`**：取 `report_events`（每次 report 的 `PlacementOutcome` 逐条落库），结构：
```json
[
  {"recorded_at": "2026-07-28T10:31:00Z", "sheet_id": 42, "sheet_title": "主城北门",
...
> v0.9.0 已实现。本段是 tracker 与后端交互的契约基线（construction-progress.md §5）。
...
| **C-11** | 加入施工联动 | 默认 tracker 启动时调 `POST /active-by-uuids` 按玩家批量解析活跃 sheet_id（无 record → skip 该玩家），无需逐玩家查 `/me/construction`；tracker 不直接落 `participants`，加入由 Web/MCDR 显式 join 或 `sheet_repo._maybe_auto_join` 自动触发 |
| **C-12** | 按材料封顶后端防线 | tracker 不需要本地维护「是否超 need」——后端 `submit_report` 逐条按 `(sheet_id, registry_id)` 跨账号合计净量封顶（§3.3）；满额 placement 返 `skipped(reason=已达材料上限)`，tracker 侧可记日志但不必重试 |
| **C-13** | 单账号单活跃项目 | tracker 经 `enforce_single_construction=True` 默认获「同账号同时仅 1 个活跃项目」保证（DB `uq_participants_active` partial unique index 兜底）；tracker 不需要自己维护单项目约束 |

**S-1 已核实**（v0.9.0）：方块放置/破坏事件名、回调签名、ctx 字段、
ServerInterface 能力、线程模型、离线 UUID 一致性——参考
...
`dormant_sources`：休眠源列表——`[{source_id, last_active_at}]`。
...
> 聚合 `placement_totals` → `BuildAScoreCalculator`。读接口已固化，归档处可直接调用。
...
| **`enforce_single_construction`** | **true** | **同账号是否同时仅 1 个活跃施工项目**：True 时 `/me/join` 冲突抛 409（要求玩家显式 `/me/switch`）；False 时自动切换。**仅约束默认 MCDR 追踪器 + Web/MCDR 的 join 流程**；`POST /report` API 零 project 校验不变——第三方/玩家客户端 mod 源仍可同时多项目上报 |
...
**约束 & 索引**：
- `uq_participants_active` UNIQUE `(web_account_id) WHERE left_at IS NULL` —— 同账号同时仅 1 个活跃项目（与 `uq_player_sources_active` 范式一致）
