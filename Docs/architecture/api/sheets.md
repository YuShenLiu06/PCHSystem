...
| GET | `/sheets/{sheet_id}` | JWT 或 service-token+UUID | `?format=csv` | 200 `SheetDetail` \| `text/csv` | 表详情；`format=csv` 返回单表 CSV。**副作用**：JSON 详情路径（非 csv、非 404）成功返回前 best-effort 记录 `users.players.last_sheet_id = sheet_id`（供 `GET /me/last_sheet` 快速重开；写失败仅记日志，不影响返回）。MCDR 同上 |
...
*最后更新：2026-07-06（快速重开上次表格：迁移 0011 + `GET /me/last_sheet` + `GET /sheets/{id}` best-effort 记录 last_sheet_id；list 增强：参与优先排序 + 阶段过滤；§2 鉴权表补 `/me/last_sheet` 双通道、§5.1 补参与优先与副作用、§5.5 新增 last_sheet 端点、§10 加迁移 0011）*
