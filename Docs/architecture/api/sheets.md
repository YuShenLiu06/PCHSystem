...
| `qty_per_unit` | int? | **子物品单位用量**（迁移 0012）：子行每个父行物品所需的子物品数量（≥1）；need_qty = qty_per_unit × 父行 need_qty（派生存储） |

**不变量（lock 模式）**：`open ⇒ claimant IS NULL ∧ delivered=0`；`claimed ⇒ claimant NOT NULL`；`done ⇒ claimant NOT NULL ∧ delivered≥need`。

**不变量（子物品，迁移 0012）**：单层（子只能挂顶层，repo 层校验 `parent.parent_row_id IS NULL`）、模式继承（父 lock→子只能 lock；父 progress→子可 lock/progress）、单位用量级联（父 need 变→子 need 重算 = qty_per_unit × 新父 need）、子行必须有 registry_id 且 qty_per_unit≥1。详见 [`data-model.md`](../data-model.md) §10.2。
...
class RowUpsertRequest(BaseModel):
    ...
    parent_row_id: int | None = None  # 子物品父行 id（可选，迁移 0012）；子行必须同时提供 registry_id 与 qty_per_unit≥1，否则 422
    qty_per_unit: int | None = None  # 子物品单位用量（可选，迁移 0012）；≥1；子行 need_qty = qty_per_unit × 父行 need_qty（派生存储，父 need 变时级联重算）
    ...
    # 子物品路径（parent_row_id 非空）：要求 registry_id 非空且 qty_per_unit≥1，否则 422；need_qty 派生忽略（由父行 need × qty_per_unit 计算）。
...
class RowDetail(BaseModel):
    ...
    qty_per_unit: int | None   # 子物品单位用量（迁移 0012）；子行每个父行物品所需的子物品数量（≥1）
...
- `qty_per_unit` 为 null 时输出空串（迁移 0012 新增列；仅子行非空且≥1）。
...
| `!!PCH sheet addsub <sheet_id> <parent_row_id> <registry_id> <qty_per_unit> [mode] [sort]` | owner | `PUT /sheets/{sheet_id}/rows` | **子物品新建**（迁移 0012）：给指定父行添加子物品（`parent_row_id`）。必须提供 registry_id 与 qty_per_unit≥1；need_qty 派生（= qty_per_unit × 父 need）。mode 可选（缺省继承父行模式）。**单层限制**：父行必须是顶层行（parent_row_id IS NULL），否则 409 |
