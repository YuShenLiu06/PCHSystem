...
| `item_name` | text | 显示名/物品名（自由文本；UNIQUE(sheet_id, item_name) 兼作 upsert 锁点）；新建缺失时后端据 `registry_id` 翻译补默认中文名 |
| `registry_id` | text? | MC 物品注册名 `namespace:path`（**隐式可空**，迁移 0009）；**一键提交（`!!PCH sheet submit`）按此精确匹配表行**；无此列值的行被一键提交跳过 |
...
| PATCH | `/sheets/{sheet_id}/rows/{row_id}/progress` | JWT·**owner** 或 service-token+UUID·owner | `RowProgressRequest{delivered_qty}` | 200 `RowDetail` | **progress 专用**：拥有者/admin 直接覆写 `delivered_qty`（绝对值，可增可减）+ 按新值重算 status，**不动 contributors**（保留上交历史）。lock 行→409（请用 `/delivery`）。值变化时通知该行所有贡献者（`sheet_progress_changed`，见 §12）。MCDR 命令：`!!PCH sheet progress`（见 §11） |
...
class RowUpsertRequest(BaseModel):
    item_name: str | None = None    # 1..64（可选）
    registry_id: str | None = None  # MC 注册名 namespace:path（可选，迁移 0009；一键提交按此匹配）
    need_qty: int = 0               # ≥0
    mode: int = 0                   # 0=lock | 1=progress
    sort_order: int = 0             # ≥0
    # model_validator：item_name 与 registry_id 至少一个非空，否则 422。
    # item_name 缺失时，后端据 registry_id 用 LangJsonTranslator（复用投影解析翻译表，
...
class RowDetail(BaseModel):
    id: int
    item_name: str
    registry_id: str | None    # MC 注册名（隐式可空，迁移 0009）；一键提交按此匹配
    need_qty: int
    mode: int                # 0|1
...
- `registry_id` 为 null 时输出空串（迁移 0009 新增列，在 `item_name` 之后）。
...
| `0009_sheet_rows_registry_id` | 给 `sheets.sheet_rows` 加列 `registry_id TEXT NULL`（隐式可空，无唯一约束；down_revision=`0008_contributed_qty`）。语义：MC 物品注册名 `namespace:path`，**一键提交 `!!PCH sheet submit` 按此精确匹配表行**。兼容旧行（null）；block id ≠ item id 时存原值不归一化。**可逆** |
...
| `!!PCH sheet progress <sheet_id> <row_id> <delivered_qty>` | owner | `PATCH /sheets/{sheet_id}/rows/{row_id}/progress` | **progress 行**拥有者修正进度（**绝对值**，可增可减、不动贡献者）；值变化通知贡献者 |
| `!!PCH sheet release <sheet_id> <row_id>` | 认领人自放 / owner 解锁 | `POST /sheets/{sheet_id}/rows/{row_id}/release` | 解除锁定（→open；progress 行清贡献者并通知） |
...
**触发规则与 category 枚举**：见 [`services/notification-service.md`](../services/notification-service.md) §3（首期 9 类 sheets 专用：`sheet_claimed`/`sheet_delivered`/`sheet_done`/`sheet_released`/`sheet_rejected`/`sheet_qty_changed`/`sheet_row_deleted`/`sheet_progress_changed`/`sheet_progress_reset`）。
...
*最后更新：2026-07-03（v0.3.0：progress 模式改为多人贡献者列表——新增 `sheet_row_contributors` 表 + `POST .../contribute` 端点 + RowDetail.contributors；progress 行 claim/delivery/reject → 409；迁移 0007；新增 `PATCH .../progress` 端点供 owner 直接覆写进度 + 迁移 0008 加 `contributed_qty`）*
...
*增量（2026-07-03，registry_id）：sheet 行加隐式可空 `registry_id`（迁移 0009）；`RowUpsertRequest`/`SheetItemIn` 的 `item_name` 改可选 + 新增 `registry_id`（至少一个，否则 422），缺失时后端据 `registry_id` 翻译补中文名；`POST /sheets/from-items` 透传 `registry_id`；CSV 导出列追加 `registry_id`（item_name 之后）；MCDR 新增 `!!PCH sheet submit`（一键提交，按 registry_id 精确匹配）/ `addhand`（手持新建行）/ `setreg`（给已有行补 registry_id）。*
