...
| `!!PCH sheet submitchest <sheet_id>` / `!!PCH sheet submitchest <sheet_id> <x> <y> <z>` | 任意玩家 | RCON `data get block` → `POST /sheets/{id}/submit-batch` | **箱子提交**（issue #48）：准星模式（无坐标）读玩家 Pos+Rotation 射线检测 6 格内首个容器；坐标模式直接读指定位置。RCON 读箱子 NBT → SNBT 解析 → `expand_items` 嵌套展开 → `submit_batch` 批量提交 → 回执复用 `_build_submit_receipt`。纯申报不清空（RS-3）。双联箱子 MVP 仅读单格（`data get block` 不含 block state `type`） |
