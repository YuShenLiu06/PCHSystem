...
- **player-less 托管账号浏览（issue #74）**：`GET /sheets`（列表）、`GET /sheets/{id}`（详情，含 csv）、`GET /sheets/{id}/managers` 用 account 级 `get_current_viewer`（双通道语义不变）——无绑定玩家的托管管理账号（`ADMIN_*` env，JWT 无 `active_uuid`）也能只读浏览（`owner=me` 返回空集、无「我参与的行」高亮、不记 `last_sheet`）；**写端点仍需玩家身份**（player-less 调用 → 401 `missing active_uuid`，前端 toast 不登出）。
...
| `!!PCH sheet submitchest <sheet_id>` / `!!PCH sheet submitchest <sheet_id> <x> <y> <z>` | 任意玩家 | RCON `data get block` → `POST /sheets/{id}/submit-batch` | **箱子提交**（issue #48）：准星模式（无坐标）射线检测 6 格内首个容器；坐标模式直接读指定位置。扫描实现由外部插件 [`chest_scanner_lib`](https://github.com/YuShenLiu06/mcdr-chest-scanner) 提供（`get_plugin_instance` 调用，v0.10.0 起取代内嵌实现）：RCON 读箱 NBT + SNBT 解析 + **双联合并** + 嵌套物品展开 → `submit_batch` 批量提交 → 回执复用 `_build_submit_receipt`。纯申报不清空（RS-3） |
| `!!submitc` / `!!submitc <sheet_id>` / `!!submitc <sheet_id> <x> <y> <z>` | 任意玩家 | `GET /me/last_sheet`（无参时）→ 同 `!!PCH sheet submitchest` | **箱子提交独立根**（第四根，issue #48）：三段式镜像 `!!submit`（无参→重开上次+准星 / `<id>`→指定表+准星 / `<id> <x y z>`→坐标）。与 `!!PCH sheet submitchest` 共用 `_submitchest_impl` |
