...
**项目三阶段生命周期**：每个 sheet（项目）有阶段状态 `collecting`（材料收集，默认）→ `constructing`（施工占位）→ `archived`（只读终态）。owner/admin 经 `POST /sheets/{id}/advance` 流转阶段；进入 `archived` 时后端渲染 markdown 归档文档原子落盘到 `ARCHIVE_ROOT`（DB 存相对路径 `archived_path`，为未来 wiki-service 同步入口），任意登录玩家可 `GET /sheets/{id}/archive` 取回 markdown。详见 §4.1「项目阶段状态机」。
...
| `archived_path` | text? | 归档 markdown 相对 `ARCHIVE_ROOT` 路径（如 `projects/42.md`）；仅 `archived` 非空，一致性 CHECK 保证 |
...
- **归档文件路径约定**：`archived_path` 存相对 `ARCHIVE_ROOT` 的 POSIX 路径（如 `projects/42.md`），单文件覆盖（archived 终态不重归档），是未来 wiki-service 读文件 GraphQL 推 wiki.js 的**单向同步入口**（首版不同步）。
...
| GET | `/sheets` | JWT 或 service-token+UUID | `?owner=me`（只看自己）+ `?status=collecting\|constructing\|archived\|active`（`active`=collecting+constructing，前端默认） | 200 `[SheetSummary]` | 列所有表（含他人表可读），可按阶段过滤。MCDR 同上 |
...
| 404 | 表/行不存在；`GET /archive` 未归档或归档文件缺失 |
...
*最后更新：2026-07-03（项目三阶段生命周期 + Markdown 归档：迁移 `0009_sheets_lifecycle`（status/archived_path/archived_at + 双 CHECK + ix_sheets_status）+ `POST /sheets/{id}/advance?to=` + `GET /sheets/{id}/archive` + `GET /sheets?status=` 过滤 + SheetSummary/Detail 三字段 + `sheet_archived` 通知 category + markdown_render Route C + `!!PCH sheet advance`）*
