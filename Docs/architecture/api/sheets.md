...
| `item_name` | text | 显示名/物品名（自由文本数据字段；`UNIQUE(sheet_id, item_name)` 作防重名约束——改名/新建撞名→409）；新建缺失时后端据 `registry_id` 翻译补默认中文名。**修改行以 `id`（主键）为定位主轴，而非 `item_name`**（issue #20：旧实现按 item_name upsert 改名会重复建行） |
