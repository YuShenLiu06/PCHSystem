...
4. **MCDR 插件挂载落主仓**：`TestServer/docker-compose.yml` 的 `../McdrPlugin/htcmc_auth:/mcdr/plugins/htcmc_auth`，`../` 相对 `TestServer/` → 解析到**主仓**插件目录。游戏内 `!!MCDR plugin reload htcmc_auth` 只读主仓工作树。
...
容器挂载的是主仓 `McdrPlugin/htcmc_auth`（§3 机制 4），`!!MCDR plugin reload` 看不到 worktree 改动。验证前先把改动同步到主仓路径：

```bash
# 二选一：直接编辑主仓对应文件 / 或 commit 后 git -C 主仓 merge <worktree分支>
grep -n '<关键改动>' /home/yushen/opt/PCHSystem/McdrPlugin/htcmc_auth/<file>.py   # 确认两边一致
...
| 变量 | 对齐要求 | 不一致的后果 |
|---|---|---|
| `MCDR_SERVICE_TOKEN` | worktree backend 的根 `.env` 值 **必须 ==** mc-test 插件 `McdrPlugin/htcmc_auth/config.json` 加载的值 | 插件 HTTP 上报被 backend 拒（401） |
...
3. **`!!MCDR plugin reload` 只读主仓路径** → worktree 改动先同步主仓再 reload（策略 D）。*why*：容器挂载的是主仓 `McdrPlugin/htcmc_auth`。
