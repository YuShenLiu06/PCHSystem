...
> **版本门禁**：release 意图 PR（命中任一：`release/**` / `hotfix/**` 分支、标题含 `release`、`release` label、改了 `plugin.json` `version`、commit 被 `pch_system-v*` tag 指向）由 [`ci.yml`](./.github/workflows/ci.yml) 的 `version-metadata` job 校验——`plugin.json.version` 须 SemVer 合法 + 已 bump + 严格前进 + CHANGELOG 有对应段，否则 block 合并；命中自动打 `release` 标签。普通 PR 不命中则跳过。直接进 main 的发版由 `release.yml` `version==tag` 兜底。脚本 [`check_version_bump.py`](./.github/scripts/check_version_bump.py)。
>
