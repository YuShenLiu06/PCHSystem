...
**v0.5.0 已发布**（2026-07-07；本轮 backend + frontend 发版，tag `backend-v0.5.0` / `frontend-v0.5.0`；MCDR 插件本轮无变更，保持 `htcmc_auth-v0.4.0`）。已落地：
...
- **投影 / 蓝图解析生成清单**：`.litematic` 投影或 Create `.nbt` 蓝图上传 → 解析 + 中文翻译 → 一键生成项目（v0.5.0 新增 `.nbt` 蓝图解析）
...
- **一键部署脚本**（v0.5.0）：`Scripts/install.sh` + `update.sh`，首次安装 / 更新一条命令（自动装 Docker、国内镜像自适应、智能重建矩阵、token 双写校验、迁移前快照）
