...
| `/sheets/*` 全套 | 表 CRUD + 行 upsert/删 + 协作（claim/delivery/contribute/release/reject/progress），`SheetList` / `SheetEditor` 消费；详见 [`api/sheets.md`](../Docs/architecture/api/sheets.md) |
...
*最后更新：2026-07-03（v0.3.0：补 sheets/parsing 消费接口 + usePolling composable + 文档索引）*
