...
| `.litematic` / `.nbt` 文件上传 UI | 文件解析（project-service 管） |
| 一次性 token → JWT 兑换页（`/auth`） | token 签发（后端管） |
| 身份管理（密码登录 / 临时账号转正 / Web 账号绑多 MC 身份 / 自助账号页 `Me.vue`） | 身份主锚与 RBAC 判定（后端 account 级为准，R-5） |
...
| **RS-6** | **临时账号引导横幅不可省**（v0.8.0） | `useAuthStore.isTemporaryAccount`（getter，读 `account.is_temporary`）为真时，`Me.vue` 必须显横幅引导 `/register` 转正；`AuthExchange.vue` 按 `is_temporary` 分流到 `/register`（临时）/ `/me`（永久）。漏掉 → 临时账号玩家看不到转正入口（数据归属 `web_account_id` 不丢，但体验断裂）。 |
...
### 依赖的其他服务
- **user-service**：`/auth/{exchange,login,refresh}`、`/me`、`/players`、`/web-accounts/{register,me}`、`/bind/{issue,confirm,claim}`
...
### 关键运行时状态
- `useAuthStore`（Pinia）：`accessToken / refreshToken / player / account: AccountBrief | null`（v0.8.0 加 `account` + `isTemporaryAccount` getter，读 `account.is_temporary`），持久化到 `localStorage`
- `utils/http.ts`：axios 实例 + 请求/响应拦截器（Bearer 注入、401 兜底）
- `useSheetDetail`（composable，v0.8.0）：三层 RBAC computed——`canManage`（tier A，owner/超管）/ `isManager`（`managers[].member_uuids ∩ viewer_uuids`）/ `canEdit`（tier B = `canManage ∨ isManager`）/ `isReadOnly`（archived）；权限契约见 [`api/sheets.md`](../Docs/architecture/api/sheets.md) §7.1（M01-M26）
...
- **临时账号看不到转正入口**：确认 `useAuthStore.isTemporaryAccount` 为真时 `Me.vue` 显横幅、`AuthExchange.vue` 分流 `/register`（RS-6）；`account` 字段未持久化则检查 `localStorage`
...
*增量（2026-07-07）：`/parsing/litematic` 页支持 `.nbt`（Create 蓝图）上传，按扩展名自动选 `POST /parsing/nbt` 端点（#5）；§4 端点表补 `/parsing/nbt`、§2 职责泛化 `.litematic / .nbt`。*
