...
http.interceptors.response.use(
  ...
  async (err: AxiosError) => {
    ...
    if (err.response?.status === 401 && canRefresh) {
      try {
        await refreshSession()
        // 重放原请求；请求拦截器会自动注入新 access token
        return http(err.config as AxiosRequestConfig)
      } catch {
    ...
    if (err.response?.status === 401) {
      // /auth/refresh 自身 401 不在此清场——它是「续签尝试失败」，由发起 refresh 的外层
      // catch 统一 clear + push，避免这里再清一次造成 double clear / double push。
      if (url !== '/auth/refresh') {
        auth.clear()
