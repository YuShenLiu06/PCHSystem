...
- **后端/前端（sheets）**：父行备齐后子行仍可认领/交付/改数（语义漏洞）——新增**父行终态冻结**：父行 `done` 后子行全部协作写 409「父行已备齐，子行已锁定」（含 addsub/reparent/`submit-batch` 整行 skip 同文案）；动态判定，父行打回或 need 上调自动解冻，存量「父 done + 子行 open」免迁移自愈；`delsub` 保留（owner 清理权）。前端子行按钮/编辑对冻结行同步禁用（R-9 仅可见性）。附带：子行派生公式 `ceil(qty×父need)` 三处内联合并为 `_derived_need_qty()` 单点（重复副本曾漂移出 `0.07×100` 进位 bug）；解除级联不再对 lock 子行逐个清贡献者（不变量恒空）；子行表单重置跟随父行 mode；清理 `findParentMode` 死代码与 addsub docstring 旧语义。
