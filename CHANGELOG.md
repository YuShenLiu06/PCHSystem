...
- **后端不可达时日志刷屏（#62）**：后端离线时通知轮询（2s/次）与施工追踪器（30s/次）每次失败都打 ERROR 日志，疯狂刷屏。两层修复：`_request` 首次网络错误保留 ERROR，后续降 DEBUG（收到任意 HTTP 响应即重置）；轮询循环加指数退避（封顶 `backoff_max_seconds`，默认 60s），进入退避时打印 `⚠ 后端不可达` 并指引 `!!PCH status`，恢复时打印 `✓ 后端已恢复`。
